...
if (isset($_GET['page']) and (int) $_GET['page'] > 0) {
    ...
    $record = $linkControl->getNumRecordByCatId($cat_id);
...
$links = $linkControl->model->listAllLinkLimitedCatId($start_record, $display, $cat_id);
$counter = 0;
$thumbOfRow = 4;
...
$last_link = "index.php?page=" . $page . '&start=' . $last_start_record.'&cat_id='.$cat_id;
$first_link = "index.php?page=" . $page . '&start=0'.'&cat_id='.$cat_id;
$previous_link = "index.php?page=" . $page . '&start=' . $previous_record.'&cat_id='.$cat_id;
$next_link = "index.php?page=" . $page . '&start=' . $next_start_record.'&cat_id='.$cat_id;
?>
<div class="row"></div>
        ...
        if ($page > 1) {
            ...
            for ($i = $start_page; $i <= $end_page; $i++) {
                if ($i == $current_page) {
                    ...
                    $link = "index.php?page=" . $page . '&start=' . (($i - 1) * $display).'&cat_id='.$cat_id;
                    ?>
