...
<html lang="en">
    <head>
        ...
        include '../View/style.php';
         ...
         if (isset($_GET['id'])) {
             $linkControl->updateVideo($_GET['id']);
         }
        ?>
    </head>
    <body> 
        ...
        <div class="container" id="nav_div">
            ...
            if (isset($_GET['id'])) {
                $hasId = true;
                $video = $linkControl->model->getVideoById($_GET['id']);
                $cat_ids[5] = array(-1, -1, -1, -1, -1);
                $cats = $linkControl->model->getCategoryByVideoId($_GET['id']);
                $i = 0;
                foreach ($cats as $cat) {
                    $cat_ids[$i] = $cat->cat_id;
                    $i++;
                }
            }
            ?>
                    ...
                    <form id="defaultForm" role="form" style="width:600px; padding-left:100px;" action="" method="POST" 
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="image_url" name="image_url" placeholder="Enter url"
                                   value="<?php if ($hasId) echo $video->image_url; ?>"
                                   >
...
<?php if ($hasId) echo $video->description; ?>
                            </textarea>
                        </div>
                        <div class="form-group">
                            <label for="exampleInputEmail1">Rating</label>
                            <input type="text" class="form-control" id="rating" name="rating" 
                                   value="<?php if ($hasId) echo $video->rating; ?>"
                                   >
                        </div>	
...
foreach ($cates as $cat) {
    if ($cat_ids[0] == $cat->id)
        echo '<option selected value="' . $cat->id . '">' . $cat->cat_name . '</option>';
    else
